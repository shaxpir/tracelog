...
class S3Uploader {
  ...
  _uploadCurrentGzipped(filePath, rawBody, cb) {
    const key = this._resolveCurrentKey(filePath) + '.gz';
  ...
  _uploadCurrentRaw(filePath, rawBody, cb) {
    const key = this._resolveCurrentKey(filePath);
  ...
  /**
   * Resolve a stable S3 key for the current (incomplete) file.
   * Uses the filename and hostname/pid as the unique identifier instead of
   * a timestamp, so periodic uploads overwrite the same S3 object rather
   * than creating duplicates.
   */
  _resolveCurrentKey(filePath) {
    const now = this._clock();
    return `${this._serviceName}/${this._environment}/${_fmtDate(now)}/current-${os.hostname()}-${process.pid}-${path.basename(filePath)}`;
  }

  /**
   * Resolve the S3 key from the template and file path.
   * Used for completed (rotated) files — each gets a unique timestamped key.
   */
  _resolveKey(filePath) {
    ...
    const vars = {
      ...
      timestamp: `${now.getFullYear()}${_pad2(now.getMonth() + 1)}${_pad2(now.getDate())}${_pad2(now.getHours())}${_pad2(now.getMinutes())}${_pad2(now.getSeconds())}`,
