...
function configLogger(opts = {}) {
  ...
  // `TRACELOG_LOGGER=false` is provided as a mechanism to *disable* a
  ...
  if (process.env.TRACELOG_LOGGER !== 'false') {
...
class Config {
  constructor(opts, logger) {
    ...
    // in `configLogger()`.
    const customLogger =
      process.env.TRACELOG_LOGGER === 'false' ? null : this.logger;
...
function normalize(opts, logger) {
  ...
  // This must be after `normalizeDurationOptions()`.
