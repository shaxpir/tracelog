...
Agent.prototype.captureEvent = function (type, opts, cb) {
  if (typeof opts === 'function') {
    cb = opts;
    opts = {};
  } else if (!opts) {
  ...
  if (!this.isStarted()) {
    if (cb) {
      cb(new Error('cannot capture event before agent is started'));
    }
    return;
  }
  ...
  // If an error was provided, also emit a full error record via captureError
  if (opts.error != null) {
    this.captureError(opts.error);
  }
...
Agent.prototype.captureEvents = function (events, cb) {
  ...
  if (!this.isStarted()) {
    if (cb) {
      cb(new Error('cannot capture events before agent is started'));
    }
    return;
  }
